...
class GreedyAgent3bisn:
    ...
    def __call__(self, observation: Observation):
        ...
        while(not (i == 0 or any(actions_risk) and min(actions_risk.values()) != 1)):
...
def greedy_agent_3bis1(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3bisn:
        cached_greedy_agents_3bisn[index] = GreedyAgent3bisn(Configuration(config),1)
...
def greedy_agent_3bis3(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3bisn:
        cached_greedy_agents_3bisn[index] = GreedyAgent3bisn(Configuration(config),3)
    return cached_greedy_agents_3bisn[index](Observation(obs))
    
def greedy_agent_3bis4(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3bisn:
        cached_greedy_agents_3bisn[index] = GreedyAgent3bisn(Configuration(config),4)
    return cached_greedy_agents_3bisn[index](Observation(obs))
    
def greedy_agent_3bis5(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3bisn:
        cached_greedy_agents_3bisn[index] = GreedyAgent3bisn(Configuration(config),5)
    return cached_greedy_agents_3bisn[index](Observation(obs))