...
class GreedyAgent3primn:
    ...
    def __call__(self, observation: Observation):
        ...
        for i in range(1,self.n+1):
            ...
            resky_move[i] = {
            position: ((resky_move[i-1][position][0]-1,resky_move[i-1][position][1]) if resky_move[i-1][position][0] != 1 else (resky_move[i-1][position][0]-1,1-(0.5*(self.n-i))
...
cached_greedy_agents_3primn = {}


def greedy_agent_3prim1(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3primn:
        cached_greedy_agents_3primn[index] = GreedyAgent3primn(Configuration(config),1)
    return cached_greedy_agents_3primn[index](Observation(obs))
    
def greedy_agent_3prim2(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3primn:
        cached_greedy_agents_3primn[index] = GreedyAgent3primn(Configuration(config),2)
    return cached_greedy_agents_3primn[index](Observation(obs))
    
def greedy_agent_3prim3(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3primn:
        cached_greedy_agents_3primn[index] = GreedyAgent3primn(Configuration(config),3)
    return cached_greedy_agents_3primn[index](Observation(obs))
    
def greedy_agent_3prim4(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3primn:
        cached_greedy_agents_3primn[index] = GreedyAgent3primn(Configuration(config),4)
    return cached_greedy_agents_3primn[index](Observation(obs))
    
def greedy_agent_3prim5(obs, config):
    index = obs["index"]
    if index not in cached_greedy_agents_3primn:
        cached_greedy_agents_3primn[index] = GreedyAgent3primn(Configuration(config),5)
    return cached_greedy_agents_3primn[index](Observation(obs))