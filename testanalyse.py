...
class GreedyAgent4n:
    def __init__(self, configuration: Configuration,n: int):
        ...
        self.n = n
        self.XYtoI = np.arange(self.configuration.rows*self.configuration.columns).reshape((self.configuration.rows,self.configuration.columns))
        self.ItoXY = np.array([row_col(pos,self.configuration.columns) for pos in range(self.configuration.rows*self.configuration.columns)])
    ...
    def __call__(self, observation: Observation):
        ...
        test = np.arange(10000).reshape((11,7))
        print(test)
